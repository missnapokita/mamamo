(function(){
  ...
  function hideOverlay(){
    ...
    // While controls are hidden, temporarily catch one tap above the iframe.
    if(tapCatcher) tapCatcher.classList.add("active");
  }
  function showOverlay(){
    ...
    // Return touch control to the actual iframe as soon as UI is visible.
    if(tapCatcher) tapCatcher.classList.remove("active");
  ...
  if(tapCatcher){
    ...
    tapCatcher.addEventListener("touchend",function(e){
      e.preventDefault();
      e.stopPropagation();
      showOverlay();
    }, {passive:false});
