(function(){
  ...
  function tickLoadingProgress(){
    ...
    var ratio=1-Math.exp(-elapsed/duration);
  ...
  function beginProgressPhase(minimum,ceiling,label,duration){
    ...
    progressPhaseDuration=Math.max(
      800,
      Number(duration)||Number(cfg.progressPhaseDurationMs)||5200
    );
  }
  ...
  function startLoadingProgress(){
    clearProgressTimer();
    displayedProgress=0;
    setLoadingProgress(0,"Preparing player…",true);
    beginProgressPhase(4,12,"Preparing request…",900);
    progressTimer=setInterval(
      tickLoadingProgress,
      Math.max(80,Number(cfg.progressTickMs)||160)
    );
  }

  function completeLoadingProgress(){
    setLoadingProgress(100,"Playable link ready");
    clearProgressTimer();
  }
  ...
  async function apiRequestOnce(forceRefresh,progressStage){
  ...
  async function apiRequest(forceRefresh,progressStage){
    ...
    for(var attempt=0;attempt<2;attempt++){
      try{
        return await apiRequestOnce(forceRefresh || attempt>0,progressStage);
      }catch(err){
        lastErr=err;
        ...
        beginProgressPhase(
          displayedProgress,
          Math.max(displayedProgress,progressStage.parsed-1),
  ...
  async function loadTerabox(options){
    ...
    try{
      var maxGenerationAttempts=options.regenerationUsed===true?1:Math.max(
        1,
      ...
      for(var generationAttempt=0;generationAttempt<maxGenerationAttempts;generationAttempt++){
        if(generationAttempt>0){
          setLoadingProgress(85,"No usable link yet. Trying a fresh one…");
        ...
        beginProgressPhase(
          ...
          generationAttempt===0?
            (options.regenerationUsed===true?"Regenerating playable link…":"Generating playable link…"):
            "Generating a fresh playable link…",
          Number(cfg.progressPhaseDurationMs)||5200
        );
        ...
        /*
         * OPTIMISTIC FAST START:
         * A syntactically valid generated URL is opened immediately. Media
         * probing and fallback selection continue without holding the dialog.
         */
        good=[optimisticItem];
        refreshQualityMenu();
        ...
        var regenerationUsed=
          options.regenerationUsed===true || generationAttempt>0;
        ...
        validateGeneratedCandidates(
          ...
          /* Preserve the previous one-time dead-link regeneration behavior. */
          if(!regenerationUsed){
          ...
          showError("The generated stream could not be opened.");
        });
