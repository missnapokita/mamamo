(function(){
  ...
  function reportDiagnostic(code,extra){
    try{
      ...
      var payload={
        ...
        cacheState:clean(extra.cacheState||cacheState||""),
  ...
  function cachedState(v){
    ...
    return s==="MEMORY-HIT" || s==="HIT" || s==="WAIT-HIT" || s==="WAIT-HIT-2" ||
           s==="HIT-AFTER-LOCK" || s==="REFRESH-WAIT-HIT" || s==="REFRESH-WAIT-HIT-2";
  }
  function sleep(ms){
  ...
  async function apiRequestOnce(forceRefresh){
    ...
    try{
      var response=await fetch(cfg.streamApi,{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({url:source,forceRefresh:!!forceRefresh}),
  ...
  async function apiRequest(forceRefresh){
     ...
     * Never loop requests indefinitely because that can increase API/Redis use.
     */
    for(var attempt=0;attempt<2;attempt++){
      try{
        return await apiRequestOnce(forceRefresh);
      }catch(err){
        lastErr=err;
  ...
  async function loadTerabox(forceRefresh){
    ...
    try{
      ...
      if(!firstPlayable){
        /*
         * If a cached worker URL died before Redis TTL, refresh exactly once.
        ...
        throw new Error(
          forceRefreshAttempted
            ? "The stream is temporarily unavailable. Please try again later."
  ...
  function manualRecovery(){
    ...
    /*
     * Prevent rapid repeated "Not Playing?" taps from generating many
     * upstream links or consuming Redis/API quota.
     */
